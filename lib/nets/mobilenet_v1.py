    ...
    def _image_to_head(self, is_training, reuse=None):
        ...
        if cfg.MOBILENET.FIXED_LAYERS > 0:
            with slim.arg_scope(mobilenet_v1_arg_scope(is_training=False)):
                net_conv, end_point = mobilenet_v1_base(net_conv,
        ...
        if cfg.MOBILENET.FIXED_LAYERS < 12:
            with slim.arg_scope(mobilenet_v1_arg_scope(is_training=is_training)):
                net_conv, end_point = mobilenet_v1_base(net_conv,
                                             ...
                                             scope=self._scope)
        end_points.update(end_point)
        self.end_points['Conv2d_5_pointwise'] = end_points['Conv2d_5_pointwise']
        self.end_points['Conv2d_13_pointwise'] = end_points['Conv2d_13_pointwise']
