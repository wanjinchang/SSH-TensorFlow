...
class SolverWrapper(object):
    ...
    def construct_graph(self, sess):
        with sess.graph.as_default():
            ...
            # Group three independent train_op into a single train op.
    ...
    def train_model_random(self, sess, max_iters):
        ...
        while iter < max_iters + 1:
            ...
            if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
                ...
                rpn_loss_cls, rpn_loss_box, total_loss, summary = self.net.train_step_with_summary_random(sess, blobs,
                                                                                                   train_op, module)
                ...
                rpn_loss_cls, rpn_loss_box, total_loss = self.net.train_step_random(sess, blobs, train_op, module)
            timer.toc()
            ...
            if iter % (cfg.TRAIN.DISPLAY) == 0:
                ...
                print('iter: %d / %d, now training module: %s, iters: %d,\n >>> total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                      '>>> rpn_loss_box: %.6f\n >>> lr: %f' % \
                      (iter, max_iters, module, iters, total_loss, rpn_loss_cls, rpn_loss_box, lr.eval()))
    ...
    def train_model_group(self, sess, max_iters):
        # Build data layers for both training and validation set
        self.data_layer = RoIDataLayer(self.roidb, self.imdb.num_classes)
        self.data_layer_val = RoIDataLayer(self.valroidb, self.imdb.num_classes, random=True)
        ...
        while iter < max_iters + 1:
            ...
            if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
                # Compute the graph with summary
                losses, summary = self.net.train_step_with_summary_group(sess, blobs, final_train_op)
                self.writer.add_summary(summary, float(iter))
                # Also check the summary on the validation set
                blobs_val = self.data_layer_val.forward()
                summary_val = self.net.get_summary(sess, blobs_val)
                ...
                # Compute the graph without summary
                losses = self.net.train_step_group(sess, blobs, final_train_op)
            timer.toc()
            ...
            # Display training information
            if iter % (cfg.TRAIN.DISPLAY) == 0:
                print('iter: %d / %d \n >>> m1_cls_loss: %.6f, m1_box_loss: %.6f, m1_total_loss: %.6f\n '
                      '>>> m2_cls_loss: %.6f, m2_box_loss: %.6f, m2_total_loss: %.6f\n '
                      '>>> m3_cls_loss: %.6f, m3_box_loss: %.6f, m3_total_loss: %.6f\n '
                      '>>> total_loss: %.6f, lr: %f' % \
                      (iter, max_iters, m1_cls_loss, m1_box_loss, m1_total_loss, m2_cls_loss, m2_box_loss, m2_total_loss,
                       m3_cls_loss, m3_box_loss, m3_total_loss, total_loss, lr.eval()))
                print('speed: {:.3f}s / iter'.format(timer.average_time))
...
def train_net(network, imdb, roidb, valroidb, output_dir, tb_dir,
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        print('Solving...')
        # train the three branches of SSH alternately randomly
        # sw.train_model_random(sess, max_iters)
        # train the three branches of SSH grouply
        sw.train_model_group(sess, max_iters)
        print('done solving')
